...
function EventScoreAndTrendUploader({ id, getevent, name, visible, close }) {
  ...
  const handelStockUpload = async (e) => {
    try {
      setLoading(true);
      await updateEvent(setLoading, id, { futureStocks: xlsxUrl });
      getevent(id);
      handelClose();
    } catch (err) {
...
export default EventScoreAndTrendUploader;
